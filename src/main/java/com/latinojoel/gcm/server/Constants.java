...
public final class Constants {
   ...
   * sending messages to this device and delete the registration_id. The client needs to re-register
   * with the GCM servers to receive notifications again.
   */
  ...
  /**
   * A particular message could not be sent because the GCM servers were not available. Used only on
   * JSON requests, as in plain text requests unavailability is indicated by a 503 response.
   */
  public static final String ERROR_UNAVAILABLE = "Unavailable";

  /**
   * A particular message could not be sent because the GCM servers encountered an error. Used only
   * on JSON requests, as in plain text requests internal errors are indicated by a 500 response.
   */
